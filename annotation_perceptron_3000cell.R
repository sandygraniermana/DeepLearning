...
logiclTransformCIPHE <- function(flow.frame, value = 500, marker = NULL){
...
taille_pop_dyademB <- function(efcsB){
  
  res = NULL
  Pop=efcsB[,"Annotation"]
  
  K<-ncol(efcsB)
  efcsB<-efcsB[,-K]
  for (i in sort(unique(Pop))){ #ds chaque pop
    inds = sample(x = 1:length(which(Pop == i)), size = 3000, replace = TRUE) #x = longueur de pop.   replace = T -> des valeurs peuvent etre reprises
    Inds = which(Pop == i) #position
    taille = Inds[inds]
    res = c(res, taille) #vecteur qui recupere resultat et taille à chq tour de boucle
  }
  ...
  return(list(data = efcsB[res,], annot = Pop[res])) #fichier avec res + pop avec res
  
}


taille_pop_dyademC <- function(efcsC){
  
  res = NULL
  Pop=efcsC[,"Annotation"]
  ...
  return(list(data = efcsC[res,], annot = Pop[res])) #fichier avec res + pop avec res
  
}


taille_pop_dyademP <- function(efcsP){
  
  res = NULL
  Pop=efcsP[,"Annotation"]
  ...
  return(list(data = efcsP[res,], annot = Pop[res])) #fichier avec res + pop avec res
...
train_taille_modif <- function(K=5){
  ...
  dataset1<-taille_pop_dyademB(efcsB = efcsB)
  dataset2<-taille_pop_dyademC(efcsC = efcsC)
  dataset3<-taille_pop_dyademP(efcsP = efcsP)
  
  Dataset = rbind(dataset1$data, dataset2$data, dataset3$data)
  DATASET = c(dataset1$annot, dataset2$annot, dataset3$annot)
  
  shuf<-sample(1:nrow(Dataset)) #melanger
  ...
  mm = model.matrix(~as.factor(DATASET)-1)  
  
  #n = neuralnet(dataset$data[,7:24], mm) #training : colonnes de 7 à 24 car de 1 à 6 : scatters à échelles différentes et col 25 - 26 : inutiles (flag et time)
  
  n = neuralnet(Dataset[,1:24], mm) #training
...
logiclTransformCIPHE1 <- function(flow.frame, value = 500, marker = NULL){
...
annot_perceptron <- function(dir_fcs){
  ...
  #t_p = cbind(tab,p)
  #write.table(tab,file="/home/data/SG/pop_proportions_FMO_Camille_Landmarks&Landmarks_Detrans_ann_percep_logTrans500_new.csv",sep=",")
  write.table(tab,file="/home/data/SG/pop_proportions_DYADEM_Data_annoted_scaffold_ann_percep_logTrans500_new.csv",sep=",")
...
pre_rec_cellAnnot_ann_annot <- function(ann, annot){
  ...
  tab = table(ann, annot)
  v_precision=v_recall=cell_pred=cell_manu=moy_harmo=rep(NA,ncol(tab))
  res = list()
  for (j in 1:ncol(tab)){
    ...
    res = data.frame(pop_names = v_names,cell_annot_pred = cell_pred, cell_annot_manu = cell_manu, recall = v_recall, precision = v_precision, moyenne_harmonique = moy_harmo)
  }
  write.table(res,file="/home/data/SG/tab_ann_annot_FMO2019_B_3000cell_v.500.csv",sep=",")
  return(res)
