...
fct<-function(dir_fcs){
  ...
  #names(flow.framesB) <- basename(list.files("/home/workspace/SG/Concat_gated_clean/Camille/", pattern = ".fcs", recursive = FALSE)) #noms des fichiers dans le chemin
  ...
  pop_GOOD<-read.table("/home/workspace/SG/Good NODE.csv",header=TRUE,sep=";",stringsAsFactors = FALSE)
...
train <- function(efcs){
  ...
  efcsB<-fct("/home/workspace/SG/Manual_Gated/Basal/")
  efcsC<-fct("/home/workspace/SG/Manual_Gated/CD3e/")
  efcsP<-fct("/home/workspace/SG/Manual_Gated/polyIC/")
...
fct_FMO<-function(dir_fcs){
  ...
  pop_GOOD<-read.table("/home/workspace/SG/Good NODE2.csv",header=TRUE,sep=";",stringsAsFactors = FALSE)
  pop<-data.frame(pop=pop,file_annot=1:nrow(pop_GOOD))
  pop_GOOD<-data.frame(pop_GOOD)
  corresp<-merge(pop,pop_GOOD,by.y="pop.Names",by.x="pop")
  corresp<-corresp[order(corresp$file_annot),]
...
pre_rec_cellAnnot_ann_annot <- function(ann, annot){
  ...
  write.table(res,file="/home/data/SG/tab_ann_annot_FMO2018_Basal1.csv",sep=",")
  return(res)
...
pre_rec_cellAnnot_a_annot <- function(a, annot){
  v_names = pop_GOOD$pop.Names
  tab = table(a, annot)
  v_precision=v_recall=cell_manu=cell_scaffold=moy_harmo=rep(NA,ncol(tab))
  res = list()
  for (j in 1:ncol(tab)){
    pop_present = row.names(tab)
    pop_present = as.numeric(pop_present)
    ...
    # mean_r = mean(v_recall[j!=11])
    # mean_p = mean(v_precision[j!=11])
    res = data.frame(pop_names = v_names,cell_annot_scaffold = cell_scaffold, cell_annot_manu = cell_manu, recall = v_recall, precision = v_precision, moyenne_harmonique = moy_harmo)
  }
  write.table(res,file="/home/data/SG/tab_a_annot_FMO2019_PolyIC1.csv",sep=",")
  return(res)
}
